...
\textbf{Definition:} a function $X$ that maps the sample space $S$ to the real line in such a way that, for every $\omega \in S$, $X(\omega)$ is a real number, is called a \emph{random variable} (or random variable for short). 
...
\[
    P(X = x) = \frac{{a \choose x} \binom{N-a}{n-x}}{Nn}
\]
...
Given $f_{X,Y} (x,y)$, to find the marginal c.d.f., integrate out the variable you wish to get rid of:
\[
